...
function send_ssh_tool ()
{
  ...
  #debug_msg  ${LINENO} "ssh -oStrictHostKeyChecking=no  -tt -i  $SSH_KEYFILE $SSH_USERID@$SSH_ADDR   sbatch   --nodes=2     --ntasks=5   --error=$errlog --output=$outlog $script_sh $SSH_SOURCEDIR $SSH_CONFIGFILE  $TOOLPREFIX $READS1   $READS2 $READSPB  ${TOOL_TYPE[$index]}   ${TOOL_TAG[$index]}    ${TOOL_NAME[$index]}   ${SSH_LOGPREFIX}stats/    ${TOOL_PARAMTERS[$index]}"
